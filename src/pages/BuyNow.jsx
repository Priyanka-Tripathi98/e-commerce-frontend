...
const API_BASE_URL = 'https://e-commerce-backend-1wch.onrender.com';
...
const BuyNow = () => {
  ...
  const paymentData = `upi://pay?pa=simmivishwakarma1910@okicici&pn=MyShop&am=${totalAmount}&cu=INR&tn=${title}`;
                ...
                {items.map((item) => (
                  <div key={item.id} className={`flex items-center gap-4 p-3 rounded-lg ${isDark ? 'bg-gray-700' : 'bg-gray-50'}`}>
                    ...
                    <div className="flex-1">
                      ...
                      {cart && <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Quantity: {item.quantity}</p>}
