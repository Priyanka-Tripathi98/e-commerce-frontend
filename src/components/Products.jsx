...
const Products = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        const res = await fetch('https://e-commerce-backend-1wch.onrender.com/product');
        if (!res.ok) {
              ...
              <div className="p-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className={`px-2 py-0.5 rounded text-xs font-bold ${isDark ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>
                    -{Math.round((1 - (productPrice || product.price || 0) / (product.originalPrice || productPrice || product.price || 1)) * 100)}%
                  </span>
                </div>
              </div>
            </article>
